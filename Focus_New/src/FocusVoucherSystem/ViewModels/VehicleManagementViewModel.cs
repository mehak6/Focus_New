...
public partial class VehicleManagementViewModel : BaseViewModel, INavigationAware
{
    ...
    /// <summary>
    /// Opens the merge vehicles dialog
    /// </summary>
    [RelayCommand]
    private async Task MergeVehicles()
    {
        try
        {
            if (CurrentCompany == null)
            ...
            if (_allVehicles.Count < 2)
            {
                StatusMessage = "At least 2 vehicles are required for merging";
                return;
            }

            var activeVehicles = _allVehicles.Where(v => v.IsActive).ToList();
            if (activeVehicles.Count < 2)
            {
                StatusMessage = "At least 2 active vehicles are required for merging";
                return;
            }

            StatusMessage = "Opening merge dialog...";

            var dialog = new Views.SimpleMergeVehiclesDialog(_dataService, CurrentCompany, activeVehicles);

            // Ensure we have a valid window owner
            ...
            if (result == true && dialog.MergeSuccessful)
            {
                StatusMessage = "Vehicles merged successfully! Refreshing data...";
                await LoadDataAsync();
                StatusMessage = "Vehicles merged and data refreshed successfully";
            }
            else
            {
        ...
        {
            StatusMessage = $"Error with merge operation: {ex.Message}";

            // Show detailed error in message box for debugging
            MessageBox.Show(
                $"An error occurred while trying to merge vehicles:\n\n{ex.Message}\n\nStack Trace:\n{ex.StackTrace}",
                "Merge Error",
                MessageBoxButton.OK,
